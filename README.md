# 치수쌤의 뮤.디.컬

> **뮤**지컬 × **디**지털 × **컬**래버레이션  
> 선생님의 뮤지컬 수업을 위한 디지털 도구 플랫폼

**Musical · Digital · Collaboration** — 뮤지컬과 디지털의 콜래버레이션으로 선생님들의 뮤지컬 수업을 돕겠다는 철학으로 만든 웹앱입니다.

🌐 **한국어 / English** 다국어 지원

---

## 소개 | About

치수쌤의 뮤.디.컬은 교육 뮤지컬 수업에 활용할 수 있는 14가지 디지털 도구를 7개 카테고리로 모아놓은 플랫폼입니다.  
교육 현장에서 바로 꺼내 쓸 수 있는 실용적인 웹 도구들을 한 곳에서 찾고 접근할 수 있습니다.

---

## 도구 목록 | Tool List

### 기반 | Foundation
| 도구 | 설명 |
|------|------|
| 감정카드 (Emotion Card) | 로버트 플루치크의 '감정의 바퀴' 이론을 바탕으로 24가지 감정을 카드 형태로 활용 |
| 우리네컷 (Our4to) | 네컷 사진 촬영 후 다양한 디자인 기능으로 장식 |
| 핑거피플 (Finger People) | 손가락 인물 카드를 만드는 캐릭터 창작 지원 도구 |

### 이야기/대본 | Story & Script
| 도구 | 설명 |
|------|------|
| 스토리텔링 카드 (Story Card) | 랜덤 이미지와 이야기 입력창으로 발상과 표현을 지원 |
| 인물도감 (Character Matrix) | 인물의 특성을 상상하고 드로잉툴로 시각화 |
| 뮤지컬 대본 노트 (Script Note) | 해설·대사·노래를 블록 형태로 작성, 저장/불러오기 지원 |

### 음악 | Music
| 도구 | 설명 |
|------|------|
| 가사 바꾸기 마법사 (Easy Lyrics) | 가사를 네모칸으로 변환하여 가창 연습 및 가사 바꾸기 지원 |
| 여기 있어 피아노 (Easy Piano) | 피아노 연주·코드·자동 반주·발성 연습을 지원하는 포켓 피아노 |
| 너를 위한 악보 (Easy Score) | 악보를 몰라도 가사 위치로 음높이를 표현하는 시각적 악보 |

### 움직임 | Movement
| 도구 | 설명 |
|------|------|
| 뮤지컬 동선 노트 (Blocking Note) | 아이콘·도형으로 무대 동선을 시각화하고 기록 |

### 무대미술 | Stage Design
| 도구 | 설명 |
|------|------|
| 마이 스테이지 (My Stage) | 블록을 쌓고 소품·배우·조명을 배치해 3D 무대 디자인 |

### 공연 | Performance
| 도구 | 설명 |
|------|------|
| 여기 있어 효과음 (Sound Box) | 다양한 효과음을 재생하고 순서대로 구성하여 공연에 활용 |
| 뮤지컬 포스터 스튜디오 (Poster Studio) | 단계별 입력으로 뮤지컬 포스터 초안 디자인 |

### 감상수업 | Appreciation
| 도구 | 설명 |
|------|------|
| 스테이지빌 (Stagebill) | STAGE 선생님들이 큐레이션한 뮤지컬 작품·추천 넘버·수업 아이디어 탐색 |

---

## 기술 스택 | Tech Stack

- **Framework**: React + Vite
- **Styling**: Tailwind CSS, shadcn/ui
- **Routing**: wouter
- **i18n**: i18next + react-i18next (한국어·English)
- **Fonts**: PretendardGOV Variable (본문), DotGothic16 (타이틀), Google Material Icons
- **Design concept**: 어린왕자(Le Petit Prince) 모티브 — 파스텔 연두 배경, 별 장식

---

## 아이디어 존중 안내 | Intellectual Property Notice

이 플랫폼에 포함된 모든 도구, 아이디어, 콘텐츠, 디자인은 **교육뮤지컬 꿈꾸는 치수쌤**이 직접 기획·개발한 창작물입니다.

- 도구의 아이디어, 구성, 명칭, 디자인을 **허락 없이 복제·변형·재배포하는 것을 금지**합니다.
- 교육 현장에서의 **개인 사용 목적**으로만 활용해 주세요.
- 인용 또는 소개가 필요한 경우 반드시 **출처를 명시**해 주세요.
- 협업 및 활용 문의: [litt.ly/chichiboo](https://litt.ly/chichiboo)

> All tools, ideas, content, and designs on this platform are original works created by **Chisu, a teacher dreaming of educational musicals**.  
> Reproduction, modification, or redistribution without explicit permission is strictly prohibited.  
> For collaboration or usage inquiries, please visit [litt.ly/chichiboo](https://litt.ly/chichiboo).

---

## 라이선스 | License

**© 교육뮤지컬 꿈꾸는 치수쌤. All rights reserved.**

이 프로젝트에는 공개 라이선스가 적용되지 않습니다. 소스 코드와 콘텐츠에 대한 모든 권리는 저작자에게 있으며, 명시적인 서면 허가 없이는 어떠한 형태로도 사용, 복사, 수정, 배포할 수 없습니다.

This project is **not licensed for public use**. All rights to the source code and content are reserved by the author. No use, copying, modification, or distribution in any form is permitted without explicit written permission.

---

*Created by. 교육뮤지컬 꿈꾸는 치수쌤*  
🔗 [litt.ly/chichiboo](https://litt.ly/chichiboo)
